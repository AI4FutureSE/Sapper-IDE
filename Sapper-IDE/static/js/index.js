...
function findBlocksByType(block, finalblock, type) {
  const result = [];
  if (block.type === type) {
  ...
  for (let i = 0; i < children.length; i++) {
    const childResult = findBlocksByType(children[i], finalchildren[i], type);
